...
/// The writable data directory, from Tauri's own path resolver. Walking up
/// from the executable only finds a dev checkout; a packaged install has no
/// such siblings.
...
/// `<app_data_dir>/export`, created on demand.
...
/// Chunks, embeds and reindexes the text. All the async work happens before
/// the db lock is taken, because a guard held across an await is not Send.
///
/// Never fails the caller: a document that saved but could not be indexed is
/// still a successful import, and shows up in search after the next reprocess.
async fn index_document_for_search(state: &State<'_, AppState>, document_id: &str, full_text: &str) {
    ...
    // A long document takes minutes to embed sequentially and commits only at
    // the end, so without this the log cannot tell "still working" from "stuck".
    if total > 50 {
...
async fn process_document(state: State<'_, AppState>, path: String) -> Result<ProcessedDocument, String> {
    ...
    // On reprocess the row keeps the existing id, so overwrite it before the
    // frontend sees it. Block-scoped because ending the guard's lexical scope is
    // what the Send analysis needs, not a bare drop.
    {
        let db = state.db.lock().unwrap();
...
/// Instant-as-you-type search, called on every debounced keystroke. An empty
/// query returns no results rather than an error.
#[tauri::command]
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // location — see app_data_root's doc comment.
            ...
            // Backfills documents that have no chunks yet. Runs once at startup in the
            // background, so the window is not held up.
