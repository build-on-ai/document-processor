...
impl Database {
    pub fn new(path: &Path) -> Result<Self> {
        ...
        // Foreign keys are off by default unless the library was built for them,
        // and SQLite reads this pragma per connection, before any table is created.
    ...
    /// Persists the document, returning the id the row was actually saved under.
    ///
    /// On reprocess the existing row's id is reused, so a caller handing the
    /// document back to the frontend must take the returned id. Document and
    /// image rows commit or roll back together.
    pub fn save_document(&self, doc: &ProcessedDocument) -> Result<String> {
        ...
        // Reprocessing replaces the image set wholesale, or old images linger
    ...
    /// Documents with text but no chunks yet; used at startup to backfill the
    /// search index.
    ...
    /// Replaces the document's chunks wholesale, or a reprocessed document
    /// accumulates a generation of rows per pass. A chunk with no embedding is
    /// still stored and still found lexically.
    ...
    fn search_lexical_ranked(&self, fts_query: &str, limit: usize) -> Result<Vec<(String, String)>> {
        // Bound as parameters, not inlined: a raw Rust string would pass the escape
        // through literally instead of the control character.
        let mut stmt = self.conn.prepare(
    ...
    /// Hybrid search: the two ranked lists fused with RRF, one best chunk per
    /// document rather than a flat list of hits.
    ///
    /// Without a query embedding it degrades to lexical only, like indexing.
...
/// Each token becomes a prefix match, so a half-typed word still finds the
/// whole one. Non-alphanumeric characters are stripped: several have
/// special meaning in FTS5 query syntax.
...
mod tests {
    ...
    // Reprocessing uses the existing document ID for image rows.
    #[test]
    fn reprocessing_document_with_images_does_not_crash() {
        ...
        // A second pass over the same path updates the existing row.
