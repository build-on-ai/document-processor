//! Hybrid search: SQLite FTS5 for the lexical half, embedding cosine
//! similarity for the semantic half, combined with Reciprocal Rank Fusion.
//!
//! Embeddings come from a local Ollama instance and nothing leaves the
//! machine. When Ollama is unreachable the semantic half is skipped and
//! search degrades to lexical only, rather than failing.

use serde::{Deserialize, Serialize};

/// A chunk ready to persist: text, its exact offsets into the source
/// document, and its embedding when one could be computed.
...
/// Without a timeout one stalled request blocks the whole sequential
/// indexing loop, and every chunk queued behind it never gets embedded.
/// A timeout costs one chunk its semantic searchability, not the app.
const EMBED_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(30);
/// RRF constant, the standard choice: large enough that one list topping
/// rank 1 does not swamp the other, small enough that rank still matters.
...
pub struct SearchHit {
    ...
    /// The best-matching chunk, exactly as it appears in the source document.
    pub snippet: String,
    /// Byte offsets of `snippet` within the document's full_text —
    /// what a "scroll to this fragment and highlight it" UI needs.
    pub start_offset: i64,
    pub end_offset: i64,
    /// Pre-split segments rather than an HTML string: the snippet is exact
    /// document content and may contain literal angle brackets, so no raw
    /// HTML crosses the IPC boundary.
...
/// Splits FTS5 highlight output, which is delimited with control characters
/// rather than HTML tags, into safe-to-render segments.
...
/// Splits text into chunks that are exact substrings of the original,
/// breaking on sentence boundaries once a chunk reaches target_size.
/// Every chunk's offsets are a real slice, which highlighting depends on.
...
/// Serialises an embedding to bytes for BLOB storage. No vector-search
/// extension at this scale: cosine similarity over a desktop-sized corpus
/// is sub-millisecond.
...
/// Circuit breaker around the embedding calls.
///
/// A per-request timeout bounds one call, but not the queue behind it: the
/// local instance serves one request at a time, so once it wedges every
/// later request waits its own full timeout. After a run of failures this
/// stops sending, and lets one probe through after a cooldown.
///
/// Shared process-wide between background indexing and live search, so a
/// query started while indexing is stuck fails fast to lexical-only.
...
/// Embeds one string through the circuit breaker above. Err means Ollama is
/// unreachable, the model is not pulled, or the breaker is open; callers
/// degrade to lexical-only rather than failing.
...
mod tests {
    ...
    fn circuit_breaker_reopens_and_restarts_cooldown_if_the_probe_fails() {
        // A wide cooldown: the assertion runs with no sleep before it, and a
        // scheduler stall on a narrow margin would flip it. Costs no extra
        // wall-clock here.
